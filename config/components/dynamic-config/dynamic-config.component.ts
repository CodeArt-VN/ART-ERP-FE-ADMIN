	...
	async loadData(event) {
		...
		for (let i = 0; i < this.optionList.length; i++) {
			...
			for (let j = 0; j < g.children.length; j++) {
					...
					c.items = []; //select
						...
						try {
							...
							if (selectOption && selectOption.Model == 'SYS_Type') {
								...
								c.DataSource = concat(
									of(c.SelectedList),
									c.SearchInput.pipe(distinctUntilChanged(),
										tap(() => c.SearchLoading = true),
										switchMap(term => this.staffProvider.search({ Take: 20, Skip: 0, IDDepartment: this.env.selectedBranchAndChildren, Term: term })
											.pipe(
												catchError(() => of([])),
								...
								c.DataSource = concat(
									...
									c.SearchInput.pipe(distinctUntilChanged(),
										tap(() => c.SearchLoading = true),
										switchMap(term => this.contactProvider.search({ Take: 20, Skip: 0, IDDepartment: this.env.selectedBranchAndChildren, Term: term })
											.pipe(
												catchError(() => of([])),
												tap(() => c.SearchLoading = false)
											)
				...
				if (!setting) {
					setting = { IDBranch: this.selectedBranch.Id, Id: 0, Code: c.Code, Value: null, ValueObject: null };
					if (c.DefaultValue) {
