...
export class UserDetailPage extends PageBase {
  ...
  preLoadData(event?: any): void {
    Promise.all([
    this.pageProvider.commonService.connect('GET', 'Account/GetRoles', null).toPromise(),
      this.accountGroup.read(),
  ]).then((values:any)=>{
      if(values && values[0]){
...
loadData(){
  ...
  loadedData(event?: any, ignoredFromGroup?: boolean): void {
    ...
    Promise.all([this.contactProvider.read({
      Take: 20,
      SkipMCP: true, SkipAddress: true, 
  ...
  .then((values : any)=>{
      ...
      this.formGroup.get('SysRoles')?.valueChanges
  ...
  savedChange(savedItem = null, form = this.formGroup) {
    if (savedItem) {
      if (form.controls.Id && savedItem.Id && form.controls.Id.value != savedItem)
        form.controls.Id.setValue(savedItem?.Id || savedItem);
